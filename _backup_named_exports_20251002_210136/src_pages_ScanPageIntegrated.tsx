...
const ScanPageIntegrated: React.FC = () => {
  ...
  const handleBarcodeDetected = async (code: string) => {
    ...
    setError(null);
    try {
      console.log("?? Analyse du code:", code, "Cat?gorie:", selectedCategory);
      ...
      console.log("? R?sultat API (normalis?):", normalizedResult);
      setShowScanner(false);
      persistAndGo(normalizedResult, code);

    } catch (err: any) {
      console.error("? Erreur:", err);
      setError(err?.response?.data?.error || err?.message || "Erreur lors de l'analyse. R?essayez.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto p-4 space-y-6">
        ...
        {showScanner && (
          <div className="mb-4">
            <BarcodeScanner isOpen={showScanner} onClose={() => setShowScanner(false)} onScanSuccess={handleBarcodeDetected}
        ...
        {selectedCategory !== "auto" && (
          <div className="bg-blue-50 border border-blue-200 p-3 rounded text-sm">
            <strong>Cat?gorie s?lectionn?e :</strong> {
              selectedCategory === "food" ? "?? Alimentaire" :
              selectedCategory === "cosmetics" ? "?? Cosm?tiques" :
              "?? D?tergents"
            }
          </div>
        )}
        ...
        <div className="text-sm text-gray-600 mt-4">
          <p>?? Conseil : S?lectionnez la cat?gorie appropri?e pour une analyse plus pr?cise.</p>
          <p>Le mode "Auto" tentera de d?tecter automatiquement le type de produit.</p>
        </div>
      </div>
    </div>
  );
...
export { ScanPageIntegrated };





export { ScanPageIntegrated as BarcodeScanPage };
