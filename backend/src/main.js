...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      '/api/dashboard',
      '/api/algolia',
        // '/api/chat',  // V1: Désactivé - Chat libre interdit
      '/api/payments',
      '/api/education'
    ]
  });
...
const routesToLoad = [
  ...
  // { path: '/api/chat', file: './routes/chat.routes.js', name: 'Chat' },  // V1: Désactivé
...
async function connectDatabase() {
  try {
    ...
    console.log('⚠️ [DB] Les routes nécessitant la DB utiliseront des mocks');
...
// ═══════════════════════════════════════════════════════════════════
// ⭐ V3.2 - AI EDUCATION ROUTES (Comparateur + Questions scientifiques)
// ═══════════════════════════════════════════════════════════════════
try {
  const aiEducationRoutes = require('./routes/aiEducation.routes');
  app.use('/api/education', aiEducationRoutes);
  console.log('✅ [ROUTE] AI Education montée sur /api/education');
  console.log('Route montée: /api/education', { path: require('path').resolve(__dirname, 'routes/aiEducation.routes.js') });
} catch (err) {
