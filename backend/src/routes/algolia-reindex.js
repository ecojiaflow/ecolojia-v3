...
router.get('/reindex/status', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      appId: ALG_APP_ID ? true : false,
      adminKey: ALG_ADMINKEY ? true : false,
      index: INDEX_NAME,
      dbConnected
    });
  } catch (e) {
...
router.post('/reindex', async (req, res) => {
  ...
  try {
    ...
    for (let skip = 0; skip < total; skip += chunkSize) {
      ...
      const url = 'https://' + ALG_APP_ID + '-dsn.algolia.net/1/indexes/' + encodeURIComponent(INDEX_NAME) + '/batch';
      const payload = { requests: records.map(r => ({ action: 'updateObject', body: r })) };
