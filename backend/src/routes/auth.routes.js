  ...
  try {
    ...
    if (error.message.includes('already registered')) {
      return res.status(409).json({
        success: false,
        error: 'Cet email est dejÂ  utilise',
        code: 'EMAIL_EXISTS'
      });
    }
    next(error);
...
router.get('/me', /* auth, */ async (req, res, next) => {
  try {
    // L'utilisateur est dejÂ  attache par le middleware auth
    const user = req.user;
...
router.put('/me', /* auth, */ async (req, res, next) => {
  try {
    ...
    // Champs autorises Â  la mise Â  jour
