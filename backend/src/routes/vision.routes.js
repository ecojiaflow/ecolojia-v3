...
if (!ENABLE_VISION || !analyzeImageHandlerOk) {
  console.warn('âš ï¸ Vision désactivée ou handler indisponible â€” montage des stubs /api/vision/*');
  ...
  router.post('/analyze-image', authMw, upload.single('image'), async (req, res) => {
    ...
    try {
      ...
      visionService
        ...
        .then(() => {
          // Le service doit lui-même mettre Ã  jour le job (ou on peut le faire ici si on reçoit un résultat)
