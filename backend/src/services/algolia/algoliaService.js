...
class AlgoliaService {
  constructor() {
    ...
    console.log('🔍 Configuration Algolia:', {
      APP_ID: this.appId ? '✅ Present' : '❌ Manquant',
      ADMIN_KEY: this.adminKey ? '✅ Present' : '❌ Manquant',
    ...
    if (this.appId && this.adminKey) {
      try {
        ...
        console.log('✅ Client Algolia initialise');
      } catch (error) {
        console.error('❌ Erreur initialisation Algolia:', error);
        this.client = null;
      }
    } else {
      console.warn('⚠️ Algolia non configure - mode degrade active');
      this.client = null;
    }
  }
  ...
  /**
   * Genere un slug à partir d'un texte
   */
  generateSlug(text) {
  ...
  /**
   * Retourne l'icone associee à une categorie
   */
  ...
  async indexProduct(product, useStaging = false) {
    ...
    try {
      ...
      console.log(`✅ Produit "${product.name || product.title}" indexe (${algoliaProduct.objectID})`);
  ...
  async indexProducts(products, useStaging = false) {
    ...
    try {
      ...
      console.log(`✅ Batch indexe: ${products.length} produits`);
      return { 
  ...
  async searchProducts(query = '', filters = {}, options = {}) {
    ...
    try {
      ...
      // MODIFICATION IMPORTANTE : Ajouter les filtres passés directement
      ...
      // Si pas de filtres directs, utiliser l'ancien système
      if (!options.facetFilters && !options.numericFilters && algoliaFilters.length > 0) {
      ...
      // DEBUG: Voir exactement ce qui est envoyé à Algolia
      console.log('DEBUG searchOptions:', JSON.stringify(searchOptions, null, 2));
  ...
  async deleteProduct(productId, useStaging = false) {
    ...
    try {
      ...
      console.log(`✅ Produit ${productId} supprime de l'index`);
    } catch (error) {
  ...
  async clearIndex(useStaging = false) {
    ...
    try {
      ...
      console.log(`⚠️ Index ${useStaging ? 'staging' : 'production'} vide completement`);
    } catch (error) {
  ...
  async testConnection() {
    if (!this.client) {
      console.error('❌ Client Algolia non initialise');
      return false;
    }
    ...
    try {
      ...
      if (stats.configured) {
        ...
        console.error('❌ Index non configure');
        return false;
      }
    } catch (error) {
      console.error('❌ Test connexion Algolia echoue:', error);
      return false;
    }
  }
  ...
  async promoteStaging() {
    ...
    try {
      ...
      console.log('✅ Staging promu en production avec succes');
    } catch (error) {
