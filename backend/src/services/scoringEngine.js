...
function calculateFoodScores(product) {
  ...
  // Bonus bio/végétal
    const bioBonus = (labels.some(l => l.includes('bio') || l.includes('organic')) || product.product_name?.toLowerCase().includes('bio') || product.product_name?.toLowerCase().includes('vegetal')) ? 15 : 0;
