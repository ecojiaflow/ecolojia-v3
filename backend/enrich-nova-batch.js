...
function loadProgress() {
  ...
  return { skip: 0, enriched: 0, l1: 0, l2: 0, l3: 0 };
}
...
async function main() {
  ...
  console.log('=== ENRICHISSEMENT NOVA (BATCH MODE) ===');
  console.log('Total:', total);
  if (progress.skip > 0) {
    console.log('REPRISE depuis:', progress.skip);
  }
  console.log('Debut:', new Date().toLocaleTimeString());
  console.log('');
  
  while (progress.skip < total) {
    // Charger un batch
    const products = await Product.find({ barcode: { $regex: /^[0-9]{8,13}$/ } })
      .sort({ _id: 1 })
      .skip(progress.skip)
      .limit(BATCH_SIZE)
      .lean();
    ...
    progress.skip += products.length;
    saveProgress(progress);
    
    const pct = (progress.skip / total * 100).toFixed(1);
    console.log(`[${pct}%] ${progress.skip}/${total} | Enrichis: ${progress.enriched} | L1:${progress.l1} L2:${progress.l2} L3:${progress.l3}`);
  }
  ...
  console.log('');
  console.log('=== TERMINE ===');
  console.log('Enrichis:', progress.enriched);
