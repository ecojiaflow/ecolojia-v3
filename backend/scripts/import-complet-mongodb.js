...
async function importAllCategories() {
  try {
    ...
    console.log(`ðŸ“¦ Import de ${detergentProducts.length} détergents...`);
    ...
    console.log(`âœ… Total produits importés: ${totalImported}`);
    ...
    console.log(`ðŸ’„ Cosmétiques: ${counts[1]} produits`);
    console.log(`ðŸ§¹ Détergents: ${counts[2]} produits`);
    console.log(`ðŸ“¦ TOTAL EN BASE: ${counts[0] + counts[1] + counts[2]} produits`);

  } catch (error) {
    console.error('âŒ Erreur générale:', error);
  } finally {
...
async function importFromOpenFoodFacts(queries, category) {
  ...
  console.log(`  âœ… ${imported} produits alimentaires importés`);
  return imported;
}
...
async function importFromOpenBeautyFacts(queries, category) {
  ...
  console.log(`  ðŸ“¦ Import de ${cosmeticProducts.length} cosmétiques...`);
  ...
  console.log(`  âœ… ${imported} produits cosmétiques importés`);
  return imported;
}
