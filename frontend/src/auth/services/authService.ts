// PATH: frontend/src/auth/services/authService.ts
...
class AuthService {
  ...
  async register(userData: RegisterRequest): Promise<void> {
    try {
      ...
      if (!response.data.success) throw new Error(response.data.message || "Erreur lors de l'inscription");
    } catch (error: any) {
  ...
  async getProfile(): Promise<User> {
    try {
      const response = await apiClient.get<{ success: boolean; user: User }>(`${this.AUTH_PREFIX}/profile`);
      if (response.data.success && response.data.user) return response.data.user;
  ...
  async resendVerificationEmail(): Promise<void> {
    try {
      ...
      if (!response.data.success) throw new Error(response.data.message || "Erreur lors de l'envoi");
    } catch (error: any) {
  ...
  async verifyEmail(token: string): Promise<void> {
    try {
      ...
      if (!response.data.success) throw new Error(response.data.message || 'Erreur de vérification');
    } catch (error: any) {
  ...
  async forgotPassword(email: string): Promise<void> {
    try {
      ...
      if (!response.data.success) throw new Error(response.data.message || 'Erreur lors de la demande');
    } catch (error: any) {
  ...
  async resetPassword(token: string, newPassword: string): Promise<void> {
    try {
      const response = await apiClient.post(`${this.AUTH_PREFIX}/reset-password`, { token, newPassword });
      if (!response.data.success) throw new Error(response.data.message || 'Erreur lors de la réinitialisation');
    } catch (error: any) {
