...
export class SecureProxyService {
  ...
  async createSecureCheckout(plan: 'monthly' | 'annual' = 'monthly'): Promise<SecureCheckoutSession> {
    try {
      ...
      if (error.response?.status === 403) {
        throw new Error('Vous devez etre connecte pour acceder Æ’Ã†'' Ã¢â‚¬â„¢Æ’Ã¢â‚¬Å¡'šÃ‚Â  cette fonctionnalite');
      }
  ...
  async sendAIMessage(message: string, context?: any): Promise<AIQuotaInfo> {
    try {
      ...
      if (error.response?.status === 403) {
        throw new Error('Quota de questions IA epuise. Passez Æ’Ã†'' Ã¢â‚¬â„¢Æ’Ã¢â‚¬Å¡'šÃ‚Â  Premium pour des questions illimitees.');
      }
  ...
  /**
   * Nettoie les ressources (Æ’Ã†'' Ã¢â‚¬â„¢Æ’Ã¢â‚¬Å¡'šÃ‚Â  appeler lors du logout)
   */
  cleanup(): void {
    if (this.tokenRefreshTimer) {
      clearTimeout(this.tokenRefreshTimer);
