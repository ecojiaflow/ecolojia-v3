...
export const useAdmin = () => {
  ...
  // Charger toutes les donnees du dashboard
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      console.log('aaaa Dashboard data loaded successfully');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors du chargement des donnees';
      setError(errorMessage);
      console.error('aa Erreur loadDashboardData:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const refreshStats = useCallback(async () => {
    try {
      ...
      console.error('aa Erreur refreshStats:', err);
    }
  }, []);

  // Declencher un nouvel import
  const triggerImport = useCallback(async (maxProducts: number = 50) => {
    ...
    try {
      ...
      // Actualiser les donnees apres l'import
      setTimeout(() => {
        loadDashboardData();
      }, 2000);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors du declenchement de l\'import';
      setError(errorMessage);
      throw err;
    }
  }, [loadDashboardData]);
  ...
  const validateProduct = useCallback(async (productId: string, status: 'verified' | 'rejected') => {
    try {
      ...
      // Mettre  jour la liste des produits localement
  ...
  // Auto-refresh des donnees toutes les 30 secondes
  useEffect(() => {
    loadDashboardData();
