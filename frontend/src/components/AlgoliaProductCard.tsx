...
const AlgoliaProductCard: React.FC<AlgoliaProductCardProps> = ({ hit }) => {
  const getConfidenceBadge = (confidence: number) => {
    if (confidence >= 0.9) {
      return { color: 'bg-green-100 text-green-800 border-green-200', text: 'IA Certifiee', icon: Shield };
    } else if (confidence >= 0.7) {
      return { color: 'bg-blue-100 text-blue-800 border-blue-200', text: 'IA Validee', icon: Zap };
    } else {
      ...
      <div className="relative h-48 bg-gradient-to-br from-eco-leaf/10 to-eco-leaf/5 flex items-center justify-center">
        {hit.images && hit.images.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-eco-leaf/50">
            ...
            <span className="text-xs">Image bientot disponible</span>
          </div>
        )}
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-eco-text/60">Score eco:</span>
        ...
        {/* Indicateur de confiance detaille */}
        {confidence > 0 && (
