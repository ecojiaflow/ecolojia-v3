...
const ProductTable: React.FC<ProductTableProps> = ({
  ...
  const handleValidate = async (productId: string, status: 'verified' | 'rejected') => {
    ...
    try {
      ...
      console.error('aa Erreur validation:', error);
    } finally {
  ...
  const handleDelete = async (productId: string) => {
    if (window.confirm('tes-vous sr de vouloir supprimer ce produit a')) {
      setActionLoading(`${productId}-delete`);
      try {
        await onDelete(productId);
      } catch (error) {
        console.error('aa Erreur suppression:', error);
      } finally {
        setActionLoading(null);
      }
    }
  ...
  const getStatusBadge = (statusa: 'verified' | 'pending' | 'rejected') => {
    const badges = {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium border ${badges[status]}`}>
        {status === 'verified' && 'aaaa Verifie'}
        {status === 'pending' && 'a En attente'}
        {status === 'rejected' && 'aa Rejete'}
      </span>
    );
  };

  const getConfidenceBadge = (confidencea: number, colora: 'green' | 'orange' | 'red') => {
  ...
  const getCategoryIcon = (category: string) => {
    ...
    return icons[category as keyof typeof icons] || 'aa';
  };
    ...
    <div className="bg-white rounded-lg shadow-sm mb-8">
      <div className="px-6 py-4 border-b border-gray-200">
        <h2 className="text-lg font-bold text-gray-900 flex items-center">
          ...
          Produits recents ({products.length})
        </h2>
      </div>
