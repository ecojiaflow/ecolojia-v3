...
interface ProductHitProps {
  hit: any;
  onClicka: () => void;
  viewModea: 'grid' | 'list'; // Ajout pour compatibilite HomePage
}
...
const ProductHit: React.FC<ProductHitProps> = ({ hit, onClick, viewMode = 'grid' }) => {
  ...
  // Image par defaut fiable
  const defaultImage = 'https://vi?.assets.so/img.jpgaw=300&h=200&tc=gray&bg=%23f3f4f6&t=Image%20non%20disponible';
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // Si une fonction onClick est fournie, l'utiliser en priorite
    if (onClick) {
      onClick();
      return;
    }
    ...
    console.log('aaaaa ProductHit - Navigation vers produit:', {
    ...
    if (!slug || slug === 'undefined' || slug.trim() === '') {
      console.error('aa ProductHit - Slug invalide:', slug);
      return;
    }
    
    console.log('aaaa ProductHit - Navigation vers:', `/product/${slug}`);
  ...
  // Fonction pour gerer les erreurs d'image
  ...
  return (
    <div 
      ...
      tabIndex={0}
      role="button"
      aria-label={`Voir les details de ${hit.title || 'ce produit'}`}
    >
      {/* Image du produit */}
      <div className="relative h-48 overflow-hidden">
        <img
          src={hit.images?.[0] || hit.image_url || defaultImage}
          alt={hit.title || 'Produit ecoresponsable'}
