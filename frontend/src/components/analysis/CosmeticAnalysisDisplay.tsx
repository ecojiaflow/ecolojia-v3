    ...
    <div className="cosmetic-analysis space-y-6">
          ...
          <div className="flex-1">
            ...
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getEndocrineRiskColor(analysis.endocrineRisk?.level || 'NONE')}`}>
              Risque : {analysis.endocrineRisk?.level === 'NONE' ? 'Aucun' : 
                        ...
                        analysis.endocrineRisk?.level === 'MODERATE' ? 'Modere' :
                        analysis.endocrineRisk?.level === 'HIGH' ? 'aaleve' : 'Tres eleve'}
            </div>
            
            {analysis.endocrineRisk?.disruptors?.length > 0 && (
              <div className="mt-4">
                <h4 className="font-medium text-gray-800 mb-2">Substances detectees :</h4>
            ...
            {analysis.endocrineRisk?.affected_systems?.length > 0 && (
              <div className="mt-3">
                <h4 className="font-medium text-gray-800 mb-1">Systemes potentiellement affectes :</h4>
            ...
            <div className="flex-1">
              ...
              <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                ...
                Risque allergique : {analysis.allergens.severity === 'LOW' ? 'Faible' :
                                   analysis.allergens.severity === 'MODERATE' ? 'Modere' : 'aaleve'}
              </div>
              ...
              <p className="text-sm text-gray-600 mt-3">
                aaa Testez le produit sur une petite zone avant utilisation
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">Ingredients synthetiques</div>
              </div>
            </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        ...
        <div className="text-xs text-gray-600 space-y-1">
          <p>aa Commission Europeenne - Liste perturbateurs endocriniens 2024</p>
          <p>aa ANSES - aavaluation securite ingredients cosmetiques</p>
          <p>aa Reglement cosmetique europeen - Allergenes obligatoires</p>
          <p>aa Base de donnees INCI internationale</p>
        </div>
      </div>
    </div>
  );
...
// ========================================
// COMPOSANT AFFICHAGE DaaTERGENTS
// ========================================
...
export const DetergentAnalysisDisplay: React.FC<DetergentAnalysisDisplayProps> = ({
  ...
  return (
    <div className="detergent-analysis space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className={`p-6 rounded-xl border-2 ${getScoreColor(analysis.environmentalScore || 0)}`}>
          <div className="text-center">
            ...
            <div className="text-xs opacity-75 mt-1">Impact ecologique</div>
          </div>
        </div>
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            ...
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getToxicityColor(analysis.aquaticToxicity?.level || 'LOW')}`}>
              Toxicite : {analysis.aquaticToxicity?.level === 'LOW' ? 'Faible' :
                         analysis.aquaticToxicity?.level === 'MODERATE' ? 'Moderee' :
                         analysis.aquaticToxicity?.level === 'HIGH' ? 'aalevee' : 'Tres elevee'}
            </div>
            
            {analysis.aquaticToxicity?.toxicIngredients?.length > 0 && (
              <div className="mt-4">
                <h4 className="font-medium text-gray-800 mb-2">Substances problematiques :</h4>
                <div className="flex flex-wrap gap-2">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">Ingredients biodegradables</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-blue-50 p-6 rounded-xl border border-blue-200">
        <h3 className="text-lg font-semibold mb-4 text-blue-800">
          ? Nos Recommandations aacologiques
        </h3>
        <ul className="space-y-2">
          {analysis.recommendations?.map((rec: string, index: number) => (
            <li key={index} className="flex items-start space-x-2 text-blue-700">
