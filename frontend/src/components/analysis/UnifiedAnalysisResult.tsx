...
export default function UnifiedAnalysisResult({ data, loading, error }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      ...
      {data?.highlights.length > 0 && <HighlightsSection highlights={data?.highlights} />}
      {data?.recommendations.length > 0 && <RecommendationsSection recommendations={data?.recommendations} />}
      <ActionsSection productId={data?.id} category={data?.category} />
    </div>
  );
}
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col items-center">
          <div className={`w-24 h-24 rounded-full ${getScoreColor(data?.score.label)} flex items-center justify-center text-white`}>
            <div className="text-center">
              <div className="text-3xl font-bold">{data?.score.label}</div>
              <div className="text-sm">{data?.score.value}/100</div>
            </div>
          </div>
...
function CategoryDetails({ data }: { data: AnalysisData }) {
  switch (data?.category) {
    case 'food':
      return <FoodDetails details={data?.details} />;
    case 'cosmetics':
      return <CosmeticsDetails details={data?.details} />;
    case 'detergents':
      return <DetergentsDetails details={data?.details} />;
    default:
      return null;
  }
}
