...
export const ProductMainActions: React.FC<ProductMainActionsProps> = ({ product }) => {
   ...
   * Version robuste avec retry pour garantir que l'élément est monté
   */
  const handleAlternatives = () => {
    const scrollToAlternatives = (attempt = 0) => {
      ...
      if (alternativesSection) {
        ...
        console.log('✅ Scroll vers alternatives effectué');
      } else if (attempt < 3) {
        // Retry après 100ms (max 3 tentatives)
        console.log(`⏳ Tentative ${attempt + 1}/3 de trouver la section alternatives`);
        setTimeout(() => scrollToAlternatives(attempt + 1), 100);
      } else {
        // Échec après 3 tentatives
        console.error('❌ Section alternatives introuvable après 3 tentatives');
  ...
  return (
    <div className="bg-white rounded-[16px] shadow-[0_2px_6px_rgba(0,0,0,0.08)] p-6 mb-6 border border-[#DDE9DA]">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        {/* Action 1 : Alternatives IA - CTA PRINCIPAL (couleur Ecolojia) */}
        <button
        ...
        {/* Action 2 : Ajouter à ma liste (couleur secondaire Ecolojia) */}
        <button
        ...
        {/* Action 3 : Ajouter à un repas (alimentaire uniquement) */}
        <button
