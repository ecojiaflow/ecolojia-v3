...
const ScoreBreakdown: React.FC<ScoreBreakdownProps> = ({ product }) => {
  ...
  // Filtrer composantes selon catÃƒÂ©gorie produit
  ...
  const components = [
    ...
    {
      key: 'nutriScore',
      title: 'QualitÃƒÂ© nutritionnelle',
      weight: '20%',
    ...
    <div className="space-y-6">
      ...
      <div className={`${getScoreBgColor(scores.overallScore)} ${getScoreBorderColor(scores.overallScore)} border rounded-lg p-6`}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-gray-900">Analyse dÃƒÂ©taillÃƒÂ©e du score</h3>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-gray-600 mt-1">SantÃƒÂ©</div>
          </div>
          <div className="text-center">
        ...
        <div className="mt-4 text-sm text-gray-700">
          <strong>ComplÃƒÂ©tude des donnÃƒÂ©es :</strong> {scores.dataCompleteness || 'Partielle'}
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            <strong>Information importante :</strong> Ces scores sont informatifs et basÃƒÂ©s sur des 
            mÃƒÂ©thodologies scientifiques (OMS, ANSES, EFSA). Ils ne remplacent pas l'avis d'un 
            professionnel de SantÃƒÂ©.
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h4 className="font-semibold text-lg text-gray-900">
          DÃƒÂ©tail des 8 composantes (mÃƒÂ©thodologie ECOLOJIA V3)
        </h4>
                    ...
                    <div className="font-semibold text-gray-900 flex items-center gap-2">
                      ...
                      {!hasScore && (
                        <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">
                          Ã¢Å¡Â Ã¯Â¸Â DonnÃƒÂ©es manquantes
                        </span>
                      )}
                    </div>
                    <div className="text-sm text-gray-600">
                      {component.data?.label || 'Non spÃƒÂ©cifiÃƒÂ©'} Ã¢â‚¬Â¢ Poids : {component.weight}
                    </div>
                  </div>
                </div>
                      ...
                      <div className="space-y-2">
                        {product.foodData.additives.map((additive: any, idx: number) => {
                          ...
                          const getRiskLabel = (risk: string) => {
                            if (risk === 'HIGH') return 'Ã°Å¸â€Â´ Risque ÃƒÂ©levÃƒÂ©';
                            if (risk === 'MEDIUM') return 'Ã°Å¸Å¸Â  Risque modÃƒÂ©rÃƒÂ©';
                            return 'Ã°Å¸Å¸Â¢ Risque faible';
                          };
                          return (
                  ...
                  {component.key === 'additives' && component.data?.dangerous?.length > 0 && (
                    <div>
                      <div className="text-sm font-semibold text-red-700 mb-1">
                        Ã¢Å¡Â Ã¯Â¸Â Additifs ÃƒÂ  risque dÃƒÂ©tectÃƒÂ©s
                      </div>
                      <div className="space-y-1">
                        {component.data.dangerous.map((additive: any, idx: number) => (
                          <div key={idx} className="text-sm text-red-600 bg-red-50 px-2 py-1 rounded">
                            {additive.code} - Risque ÃƒÂ©levÃƒÂ©
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <h4 className="font-semibold text-gray-900 mb-2 flex items-center gap-2">
          ...
          mÃƒÂ©thodologie scientifique
        </h4>
        <div className="text-sm text-gray-600 space-y-1">
          <p>Ã¢â‚¬Â¢ <strong>8 composantes</strong> pondÃƒÂ©rÃƒÂ©es selon leur impact SantÃƒÂ©/environnement</p>
          <p>Ã¢â‚¬Â¢ <strong>Sources officielles</strong> : OMS, ANSES, EFSA, ADEME, SantÃƒÂ© Publique France</p>
          <p>Ã¢â‚¬Â¢ <strong>Version 3.0.0</strong> - CalculÃƒÂ© le {new Date(scores.calculatedAt || Date.now()).toLocaleDateString('fr-FR')}</p>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="text-sm text-yellow-800">
          <strong>Ã¢Å¡Â Ã¯Â¸Â Rappel lÃƒÂ©gal :</strong> ECOLOJIA n'est pas un dispositif mÃƒÂ©dical. Pour un suivi 
          nutritionnel personnalisÃƒÂ© ou en cas de pathologie (diabÃƒÂ¨te, allergies, etc.), consultez un 
          professionnel de SantÃƒÂ© diplÃƒÂ´mÃƒÂ© (mÃƒÂ©decin, nutritionniste, diÃƒÂ©tÃƒÂ©ticien).
        </div>
      </div>
    </div>
  );
