...
const ProductPage: React.FC = () => {
  ...
  const overallScore = Math.round((product.scores.healthScore + product.scores.environmentScore) / 2);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-lg border ${getScoreBgColor(product.scores.healthScore)}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Heart className="w-5 h-5 text-red-500" />
                      <span className="font-medium">Santé</span>
                    </div>
                    <span className={`text-2xl font-bold ${getScoreColor(product.scores.healthScore)}`}>
                      {product.scores.healthScore}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1">Impact sur votre santé</p>
                </div>

                <div className={`p-4 rounded-lg border ${getScoreBgColor(product.scores.environmentScore)}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Leaf className="w-5 h-5 text-green-600" />
                      <span className="font-medium">Environnement</span>
                    </div>
                    <span className={`text-2xl font-bold ${getScoreColor(product.scores.environmentScore)}`}>
                      {product.scores.environmentScore}
                    </span>
                  </div>
        ...
        {/* Nutrition */}
        {product.nutrition && product.category === 'food' && (
          <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
