/**
 * ProductPage.tsx — ECOLOJIA Mini-Spec V1
 *
 * RÈGLE DÉCISION-FIRST :
 * Niveau → Réflexe → Action au-dessus de tout
 * Métriques uniquement en détails (accordéon)
 *
 * @version 5.0.0 - Mini-Spec V1 compliant
 */
...
export default function ProductPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 grid place-items-center p-4">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center">
          ...
          <button onClick={() => nav("/search")} className="mt-6 w-full rounded-xl px-5 py-3 text-sm font-semibold text-white bg-slate-900 hover:bg-slate-800">
  ...
  const isBio = false; // À extraire des labels si disponible

  // ============================================================================
  // RENDER — Mini-Spec V1 Order
  // ============================================================================
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-slate-200">
        <div className="mx-auto max-w-2xl px-4 py-3 flex items-center justify-between">
          <button onClick={() => nav(-1)} className="p-2 -ml-2 rounded-xl hover:bg-slate-100">
            <ArrowLeft className="h-5 w-5 text-slate-700" />
          </button>
          <span className="text-sm font-medium text-slate-600">Fiche produit</span>
          <button onClick={onShare} className="p-2 -mr-2 rounded-xl hover:bg-slate-100">
      ...
      <div className="mx-auto max-w-2xl px-4 py-5 space-y-4">
        ...
        {/* 1. HERO — Nom + Marque + Image (PAS de métriques) */}
        ...
        {/* 2. BLOC DÉCISION — Niveau → Réflexe → Action */}
        <DecisionBlock
          level={level}
        ...
        {/* 3. POURQUOI CE NIVEAU — 3 puces max */}
        <WhyThisLevel
          flags={flags}
          nova={nova}
        ...
        {/* 7. DÉTAILS — Accordéon fermé (score + jauges ici) */}
        <DetailsAccordionV2
