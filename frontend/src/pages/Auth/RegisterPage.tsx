...
const RegisterPage: React.FC = () => {
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    try{ await authService.register({ email, password }); nav("/dashboard"); }
    catch(e:any){ setError(e?.message||"Échec d'inscription"); }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--eco-bg)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div className="eco-card" style={{ width:"100%", maxWidth:460, padding:24 }}>
        <h1 style={{ margin:0, fontSize:28, fontWeight:800 }}>Créer un compte</h1>
        <p style={{ marginTop:8, color:"#607069" }}>Bienvenue dans ECOLOJIA 🌿</p>
        ...
        <p style={{ marginTop:16, color:"#607069" }}>
          Déjà inscrit ? <Link to="/login" style={{ color:"#2c6e2f", fontWeight:700 }}>Se connecter</Link>
        </p>
      </div>
    </div>
  );
