...
const ScanPage: React.FC = () => {
  ...
  const persistAndGo = (result: any, code?: string) => {
    try { sessionStorage.setItem("ecolojia:lastResult", JSON.stringify(result)); } catch {}
  ...
  const handleBarcodeDetected = async (code: string) => {
    ...
    try {
      ...
      const res = await productService.analyze({ barcode: code, category: "food" });
      const result = (res && (res as any).data && (res as any).data.product) ? (res as any).data : res;
      console.log("✅ Résultat API (normalisée):", result);
      setShowScanner(false);
      persistAndGo(result, code);
      // Fallback hard redirect si le routeur ne navigue pas
      setTimeout(() => {
        try { if (!location.pathname.includes("/results")) { window.location.href = `/results?barcode=${encodeURIComponent(code)}`; } } catch {}
      }, 80);
