...
import { useQuery } from '@tanstack/react-query'; // aaaa Correction ici
import {
  AlertTriangle,
  Droplet,
  Leaf,
...
const DetergentAnalysisPage: React.FC = () => {
  ...
  const getRatingColor = (rating: string) => {
    switch (rating) {
      ...
      case 'Modéré': return 'text-green-500';
  ...
  const getRatingBgColor = (rating: string) => {
    switch (rating) {
      ...
      case 'Modéré': return 'bg-yellow-50';
            ...
            >
              <div className="px-6 pb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm font-medium">aacologique</p>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRatingColor(analysis.details.biodegradability.rating)}`}>
                      {analysis.details.biodegradability.rating === 'excellent' ? 'Excellent' :
                       ...
                       analysis.details.biodegradability.rating === 'Modéré' ? 'Modere' : 'Faible'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRatingColor(analysis.details.cdv.rating)}`}>
                      {analysis.details.cdv.rating === 'excellent' ? 'Tres faible' :
                       ...
                       analysis.details.cdv.rating === 'Modéré' ? 'Modere' : 'aaleve'}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600">
              ...
              <div className="mt-6">
                ...
                <div className={`p-4 rounded-lg ${analysis.details.phosphates.present ? 'bg-orange-50' : 'bg-green-50'}`}>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Conformite EU: {analysis.details.phosphates.euCompliant ? 'aaaa Conforme' : 'aaaaa Non conforme'}
                  </p>
                </div>
              </div>
            </motion.div>
                    ...
                    <div key={index} className="p-3 rounded-lg bg-orange-50">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            Type: {irritant.type} aa Sévérité: {
                              irritant.severity === 'high' ? 'aalevee' :
                              irritant.severity === 'Modéré' ? 'Moderee' : 'Faible'
                            }
                          </p>
      ...
      {(analysis.certifications.eco.length > 0 || 
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-4">
            {analysis.certifications.eco.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold text-gray-700 mb-2">aacologiques</h3>
