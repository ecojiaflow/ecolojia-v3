...
export function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formDat?.password.length < 8) {
    ...
    if (!formDat?.confirmPassword) {
      newErrors.confirmPassword = 'Veuillez confirmer votre mot de passe';
    } else if (formDat?.password !== formDat?.confirmPassword) {
      newErrors.confirmPassword = 'Les mots de passe ne correspondent pas';
    }

    // Conditions
    if (!formDat?.acceptTerms) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authService.register({
        ...
        acceptTerms: formDat?.acceptTerms,
        marketingOptIn: formDat?.marketingOptIn
      });
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start">
              <input
                ...
                name="marketingOptIn"
                type="checkbox"
                checked={formDat?.marketingOptIn}
                onChange={handleChange}
                className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded mt-0.5"
              />
