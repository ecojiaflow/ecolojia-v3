...
const AdminDashboard: React.FC = () => {
  ...
  const handleImport = async () => {
    try {
      ...
      alert(`aaaa Import demarre: ${result.message}`);
    } catch (err) {
      console.error('aa Erreur import:', err);
    }
  };
            ...
            <div className="flex items-center space-x-4">
              ...
              {lastUpdate && (
                <div className="text-sm text-gray-500">
                  Derniere MJ: {lastUpdate.toLocaleTimeString()}
                </div>
              )}
            </div>
        ...
        {loading && !stats && (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">Chargement des donnees admin...</p>
            </div>
          </div>
        )}
        ...
        {stats && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <StatsCard
                title="Categories"
                value={Object.values(stats.productsByCategory).reduce((a, b) => ? + b, 0)}
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <h2 className="text-lg font-bold text-gray-900 mb-4">Repartition par categories</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {Object.entries(stats.productsByCategory).map(([category, count]) => (
                  <div key={category} className="text-center p-4 bg-gray-50 rounded-lg">
                    <div className="text-2xl mb-2">
                      {category === 'alimentaire' && 'aa'}
                      {category === 'cosmetic' && 'aaaaa'}
                      {category === 'detergent' && ''}
                    </div>
