...
export const ManualAnalysisPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formDat?.name || !formDat?.ingredients) {
    ...
    try {
      const result = await aiAnalysisService.analyzeProduct({
        ...
        name: formDat?.name,
        brand: formDat?.brand,
        ingredients: formDat?.ingredients,
        image: formDat?.image
      });
  ...
  const selectedCategory = categories.find(c => c.id === formDat?.category)!;
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <div>
              <label htmlFor="ingredients" className="block text-sm font-medium text-gray-700 mb-1">
                {formDat?.category === 'cosmetics' ? 'Liste INCI' : 'Liste des ingredients'} <span className="text-red-500">*</span>
              </label>
              <textarea
                id="ingredients"
