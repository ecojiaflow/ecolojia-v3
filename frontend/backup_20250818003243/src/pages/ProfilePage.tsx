  ...
  const { data: userProfile, isLoading, error } = useQuery<UserProfile>({
    ...
    queryFn: async () => {
      try {
        ...
        if (response.data && !response.dat?.data && !response.dat?.user) {
          // Adapter la structure pour correspondre Â  UserProfile
          const userData = response.data;
          return {
            profile: {
              id: userDat?._id || userDat?.id,
              email: userDat?.email,
              name: userDat?.name || userDat?.email.split('@')[0],
              avatar: userDat?.avatar,
              emailVerified: userDat?.emailVerified || false,
              createdAt: userDat?.createdAt,
              lastLogin: userDat?.lastLogin
            },
            plan: userDat?.subscription || {
              code: 'free',
              status: 'active',
              periodEnd: undefined
            },
            limits: userDat?.limits || {
              scansPerMonth: 30,
              aiChatsPerMonth: 5,
              exportPerMonth: 0,
              favoritesMax: 10
            ...
            aiPreferences: userDat?.aiPreferences || {}
          };
        }
        
        // Si la structure est differente
        return response.dat?.data || response.dat?.user || response.data;
      } catch (error: any) {
