...
export const useQuota = () => {
  ...
  const convertFromLegacyQuota = useCallback((legacyData: DetailedQuotaData): QuotaStatus => {
    return {
      ...
      scans: {
        used: legacyDat?.used_analyses,
        limit: legacyDat?.daily_limit,
        remaining: legacyDat?.remaining_analyses,
        resetDate: legacyDat?.reset_time
      },
      aiQuestions: {
        dailyUsed: 0,
        dailyLimit: 3,
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          if (dat?.success) {
            setQuotaStatus(dat?.data); // Nouveau status apres increment
            return true;
          }
        }
      } catch (newApiError) {
