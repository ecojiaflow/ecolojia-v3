...
export function useProductAnalysis(options: UseProductAnalysisOptions = {}) {
  ...
  const analyzeManual = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${dat?.dat?.name} ? ete analyse avec succes.`,
      });
      ...
      if (options.redirectOnSuccess) {
        const route = getResultRoute(dat?.dat?.category, dat?.dat?.id);
        navigate(route);
      }
    },
    onError: (error) => {
  ...
  const analyzeBarcode = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${dat?.dat?.name} ? ete identifie et analyse.`,
      });
  ...
  const analyzeAuto = useCallback(async (data: {
    ...
    const category = detectProductCategory(dat?.name, dat?.ingredients || dat?.composition);
    
    const request: AnalysisRequest = {
      mode: 'manual',
      category,
      name: dat?.name,
      ingredients: dat?.ingredients || dat?.composition,
      barcode: dat?.barcode
    };
  ...
  const reanalyze = useCallback(() => {
    if (lastAnalysis?.data) {
      const request: AnalysisRequest = {
        mode: 'manual',
        category: lastAnalysis.dat?.category,
        name: lastAnalysis.dat?.name,
        ingredients: lastAnalysis.dat?.details.ingredientsText || lastAnalysis.dat?.details.composition
      };
