...
export const useUserAnalytics = () => {
  ...
  const evolution = useCallback((): ScoreEvolution[] => {
    ...
    return Object.entries(byDay).map(([dateStr, data]: [string, any]) => ({
      ...
      healthScore: Math.round(dat?.scores.reduce((sum: number, score: number) => sum + score, 0) / dat?.scores.length),
      novaGroup: Math.round(dat?.novaGroups.reduce((sum: number, nova: number) => sum + nova, 0) / dat?.novaGroups.length),
      scansCount: dat?.count,
      productName: dat?.products[0]
