...
router.post('/', validateApiKey, async (req: express.Request, res: express.Response) => {
  try {
    ...
    const product = await prism?.product.create({
      data: {
        name: validatedDat?.name,
        description: validatedDat?.description,
        price: validatedDat?.price,
        currency: validatedDat?.currency,
        ethicalScore: 0, // Will be updated by AI enrichment
        affiliateLink: validatedDat?.affiliate_url,
        category: validatedDat?.category,
        tags: validatedDat?.tags,
        certifications: validatedDat?.certifications
      }
    });
    ...
    if (process.env.N8N_WEBHOOK_URL) {
      try {
        await axios.post(process.env.N8N_WEBHOOK_URL, {
          ...
          lang: validatedDat?.lang,
          zones_dispo: validatedDat?.zones_dispo
        });
      } catch (error) {
