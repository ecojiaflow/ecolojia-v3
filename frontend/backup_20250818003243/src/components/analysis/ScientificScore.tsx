  ...
  const renderNovaDetails = () => {
    ...
    return (
      <div className="mt-3 p-3 border rounded-lg bg-gray-50">
        ...
        <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${novaColors[nov?.group as keyof typeof novaColors]}`}>
          Groupe {nov?.group} - {nov?.group === 1 ? 'Non transforme' : nov?.group === 2 ? 'Peu transforme' : nov?.group === 3 ? 'Transforme' : 'Ultra-transforme'}
        </div>
        {nov?.reasoning && nov?.reasoning.length > 0 && (
          <div className="mt-2">
            <p className="text-xs text-gray-600 mb-1">Detection :</p>
    ...
    return (
      <div className="mt-3 p-3 border rounded-lg bg-gray-50">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-medium text-sm">Additifs Detectes</span>
        </div>
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-gray-600">Controverses</div>
          </div>
        </div>
        {additives.detected_additives && additives.detected_additives.length > 0 && (
          <div>
            <p className="text-xs text-gray-600 mb-2">Additifs identifies :</p>
              ...
              {additives.detected_additives.slice(0, 3).map((additive, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs ${
                    ...
                    {additive.risk_level === 'high' ? 'aaleve' : additive.risk_level === 'medium' ? 'Modere' : 'Faible'}
                  </span>
                </div>
              ))}
            </div>
