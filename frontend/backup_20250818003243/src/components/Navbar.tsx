...
interface SearchSuggestion {
  query: string;
  typea: 'product' | 'brand' | 'category' | 'ingredient';
  icona: string;
  categorya: string;
  scorea: string;
}
...
class NavbarSearchService {
  ...
  async getSuggestions(query: string): Promise<SearchSuggestion[]> {
    const popularSuggestions = [
      // Alimentaire
      { query: 'nutella bio', icon: 'aaa', category: 'Alimentaire', score: 'A' },
      { query: 'yaourt nature danone', icon: 'aaaa', category: 'Alimentaire', score: 'A' },
      { query: 'pain complet bio', icon: 'aaa', category: 'Alimentaire', score: 'A' },
      { query: 'coca-cola zero', icon: 'aaa', category: 'Alimentaire', score: 'D' },
      
      // Cosmetiques
      { query: 'shampoing l\'oreal sans sulfate', icon: 'aaa', category: 'Cosmetiques', score: 'B' },
      { query: 'creme nivea sans parabenes', icon: 'aaaaa', category: 'Cosmetiques', score: 'B' },
      { query: 'dentifrice signal bio', icon: 'aaa', category: 'Cosmetiques', score: 'A' },
      { query: 'deodorant dove', icon: 'aaaaaa', category: 'Cosmetiques', score: 'C' },
      
      // Detergents
      { query: 'lessive ariel pods', icon: 'aaa', category: 'Detergents', score: 'C' },
      { query: 'liquide vaisselle paic', icon: 'aaaaaa', category: 'Detergents', score: 'B' },
      { query: 'nettoyant ajax', icon: 'aaa', category: 'Detergents', score: 'C' },
      { query: 'lessive ecologique arbre vert', icon: 'aaaaaa', category: 'Detergents', score: 'A' },
      
      // Marques populaires
      { query: 'produits bjorg', icon: 'aaaaaa', category: 'Marque Bio' },
      { query: 'produits la vie claire', icon: 'aaaaaa', category: 'Marque Bio' },
      { query: 'produits carrefour bio', icon: 'aaaaaaa', category: 'Marque Bio' }
    ];
    ...
    if (queryLower.includes('bio') || queryLower.includes('sans')) {
      contextual.push(
        { query: `${query} NOVA 1`, icon: 'aaaaaa', category: 'Alimentaire sain' },
        { query: `${query} nutri-score A`, icon: 'aaaaaa', category: 'Top sante' }
      );
    }

    if (queryLower.includes('shampoing') || queryLower.includes('savon')) {
      contextual.push(
        { query: `${query} hypoallergenique`, icon: 'aaaaaa', category: 'Peau sensible' },
        { query: `${query} vegan`, icon: 'aaaaaa', category: 'aaathique' }
      );
    }
...
const Navbar: React.FC = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (quickSearchQuery.trim()) {
      setShowSearchDropdown(false);
      navigate(`/searchaq=${encodeURIComponent(quickSearchQuery.trim())}`);
    }
  };
  ...
  const handleSuggestionClick = (suggestion: SearchSuggestion) => {
    ...
    setShowSearchDropdown(false);
    navigate(`/searchaq=${encodeURIComponent(suggestion.query)}`);
  };
  ...
  const getScoreColor = (scorea: string) => {
    switch(score) {
                ...
                <div className="p-4 border-b border-[#DDE9DA] bg-gradient-to-r from-[#E9F8DF] to-[#F7F9F4]">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-2 text-xs text-gray-600">
                      ...
                      <span>IA Scientifique "a Temps reel</span>
                    </div>
                  </div>
                    ...
                    <div className="border-t border-[#DDE9DA] p-3 bg-[#F7F9F4]">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs text-gray-500">
                          Appuyez <kbd className="px-1 bg-[#DDE9DA] rounded">aaaa</kbd> pour rechercher
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg py-2 border border-[#DDE9DA]">
                    <div className="px-4 py-2 border-b border-[#DDE9DA]">
                      ...
                      <p className="text-xs text-gray-500">
                        Plan {user?.plan === 'premium' ? 'Premium aaa' : 'Gratuit'}
                      </p>
                    </div>
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-[#DDE9DA] pt-3 mt-3">
                <div className="px-3 py-2">
                  <h3 className="text-sm font-semibold text-[#3B3B3B] mb-2">aaaa IA Scientifique</h3>
