...
export const ManualSearch: React.FC<ManualSearchProps> = ({
  ...
  const debouncedSearchTerm = useDebounce(`${formDat?.name} ${formDat?.brand}`.trim(), 300);

  // Auto-save dans localStorage
  useEffect(() => {
    if (autoSave && formDat?.name) {
  ...
  const searchProducts = async () => {
    ...
    try {
      const results = await searchService.searchProducts(debouncedSearchTerm, {
        hitsPerPage: 5,
        facetFilters: formDat?.category ? [`category:${formDat?.category}`] : []
      });
  ...
  const validateForm = (): boolean => {
    ...
    if (!formDat?.name.trim()) {
      newErrors.name = 'Le nom du produit est requis';
    }
    
    if (!formDat?.brand.trim()) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...formData,
      ingredients: formDat?.ingredients.trim(),
      barcode: formDat?.barcode.trim()
    });
  };
