...
const RadarChart = ({ data, maxValue = 100, size = 300 }) => {
  ...
  const points = dat?.map((item, index) => 
    calculatePoint(item.value, index, dat?.length)
  );
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width={size} height={size} className="overflow-visible">
        ...
        {/* Lignes radiales */}
        {dat?.map((_, index) => {
          const angle = (Math.PI * 2 * index) / dat?.length - Math.PI / 2;
        ...
        {/* Labels des axes */}
        {dat?.map((item, index) => {
          const labelPoint = calculateLabelPoint(index, dat?.length);
          return (
            <g key={index}>
