...
export const DailyAnalysesChart: React.FC<DailyAnalysesChartProps> = ({
  ...
  const totalAnalyses = dat?.reduce((sum, day) => sum + day.count, 0);
  const avgDailyAnalyses = Math.round(totalAnalyses / dat?.length || 0);
  const maxAnalyses = Math.max(...dat?.map(d => d.count));
  const avgGlobalScore = Math.round(
    dat?.reduce((sum, day) => sum + (day.avgScore || 0), 0) / dat?.filter(d => d.avgScore > 0).length || 0
  );
