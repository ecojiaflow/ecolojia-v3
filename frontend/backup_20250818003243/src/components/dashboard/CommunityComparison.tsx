...
export const CommunityComparison: React.FC = () => {
  ...
  const radarData = [
    {
      subject: 'Score Sante',
      user: comparisonDat?.user.avgHealthScore,
      community: comparisonDat?.community.avgHealthScore,
      fullMark: 100
    },
    {
      subject: 'Analyses/Mois',
      user: Math.min(comparisonDat?.user.totalAnalyses, 100),
      community: Math.min(comparisonDat?.community.avgAnalysesPerUser, 100),
      fullMark: 100
    },
    {
      subject: 'Diversite',
      user: comparisonDat?.user.categoriesAnalyzed * 33.33,
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <motion.div
        ...
        className="mb-8"
      >
        <div className={`bg-gradient-to-r ${getRankColor(comparisonDat?.percentile)} rounded-2xl p-6 text-white`}>
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                <span className="text-4xl">{getRankIcon(comparisonDat?.percentile)}</span>
                <div>
                  <h3 className="text-2xl font-bold">{getRankTitle(comparisonDat?.percentile)}</h3>
                  <p className="text-white/80">Top {100 - comparisonDat?.percentile}% des utilisateurs</p>
                </div>
              </div>
              <p className="text-white/90">{comparisonDat?.message}</p>
            </div>
            <div className="text-right">
              <div className="text-5xl font-bold">{comparisonDat?.percentile}a</div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-gray-800">{comparisonDat?.user.avgHealthScore}/100</div>
              <div className="text-sm text-gray-500">
                vs {comparisonDat?.community.avgHealthScore}/100 communaute
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-gray-800">{comparisonDat?.user.totalAnalyses}</div>
              <div className="text-sm text-gray-500">
                vs {Math.round(comparisonDat?.community.avgAnalysesPerUser)} en moyenne
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-blue-600">#{Math.round((100 - comparisonDat?.percentile) * comparisonDat?.community.totalUsers / 100)}</div>
              <div className="text-sm text-gray-500">
                sur {comparisonDat?.community.totalUsers} utilisateurs
              </div>
            </div>
          </div>
      ...
      {selectedView === 'categories' && (
        ...
        >
          <div className="space-y-4">
            {Object.entries(comparisonDat?.user.scoreByCategory || {}).map(([category, score]) => (
              <div key={category} className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="relative">
                  ...
                  <div 
                    ...
                    style={{ left: `${comparisonDat?.community.avgHealthScore}%` }}
                  >
      ...
      {selectedView === 'distribution' && (
        ...
        >
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={comparisonDat?.community.scoreDistribution}>
                ...
                <Bar dataKey="percentage" name="% d'utilisateurs">
                  {comparisonDat?.community.scoreDistribution.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={
                        entry.range.includes(comparisonDat?.user.avgHealthScore.toString()) ? '#3b82f6' 
                          : '#e5e7eb'
                      } 
                    />
                  ))}
