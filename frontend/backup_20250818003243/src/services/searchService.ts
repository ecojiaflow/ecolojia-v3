...
class SearchService {
  async search(params: SearchParams): Promise<SearchResponse> {
    ...
    try {
      ...
      if (response.data?.success && response.data?.products) {
        products = response.dat?.products;
      ...
      const hits = filteredProducts.map((product: any, index: number) => {
        ...
        if (product.analysisData) {
          hit.healthScore = product.analysisDat?.healthScore || 0;
          hit.environmentScore = product.analysisDat?.environmentScore || 0;
          hit.socialScore = product.analysisDat?.socialScore;
          
          // Si les scores detailles existent
          if (product.analysisDat?.scores) {
            hit.healthScore = product.analysisDat?.scores.healthScore || hit.healthScore;
            hit.environmentScore = product.analysisDat?.scores.environmentScore || hit.environmentScore;
          }
          
          // Nutriscore et ecoscore depuis details
          if (product.analysisDat?.details) {
            hit.nutriScore = product.analysisDat?.details.nutriscore;
            hit.ecoScore = product.analysisDat?.details.ecoscore;
          }
        }
  ...
  async searchByBarcode(barcode: string): Promise<ProductHit | null> {
    try {
      ...
      if (!response.data || response.dat?.success === false) {
        return null;
      }
      
      const product = response.dat?.product || response.data;
