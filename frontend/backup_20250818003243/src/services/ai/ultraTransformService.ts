...
class UltraTransformService {
  ...
  constructor() {
    this.baseUrl = import.met?.env.VITE_API_URL || 'https://ecolojia-backend-working.onrender.com';
    this.headers = {
  ...
  async analyzeUltraTransformation(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorDat?.error || errorDat?.message || `Erreur HTTP ${response.status}`);
      }
      ...
      if (!dat?.success) {
        ...
        throw new Error(dat?.message || 'Erreur analyse ultra-transformation');
      }
      
      // Extraction du resultat selon la structure de reponse
      const result = dat?.analysis || data;
