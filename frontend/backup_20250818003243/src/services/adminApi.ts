...
const API_BASE_URL = import.met?.env.VITE_API_URL || 'https://ecolojia-backend-working.onrender.com';
...
export class AdminApiService {
  ...
  static async getDashboardStats(): Promise<AdminStats> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la recuperation des statistiques');
      }
  ...
  static async getRecentProducts(limit: number = 10): Promise<RecentProduct[]> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la recuperation des produits');
      }
  ...
  static async getImportLogs(): Promise<ImportLog[]> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la recuperation des logs');
      }
  ...
  static async triggerImport(maxProducts: number = 50): Promise<{ message: string; importId: string }> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors du declenchement de l\'import');
      }
  ...
  static async getImportProgress(importId: string): Promise<ImportProgress> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la recuperation du progres');
      }
  ...
  static async deleteProduct(productId: string): Promise<{ message: string }> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la suppression');
      }
  ...
  static async validateProduct(productId: string, status: 'verified' | 'rejected'): Promise<{ message: string }> {
    try {
      ...
      if (!response.dat?.success) {
        throw new Error(response.dat?.error || 'Erreur lors de la validation');
      }
