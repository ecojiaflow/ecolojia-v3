...
class AnalysisService {
  ...
  async analyzeProduct(request: AnalysisRequest): Promise<AnalysisResponse> {
    try {
      ...
      if (error.response?.data?.error) {
        throw new Error(error.response.dat?.error.message || 'Erreur lors de l\'analyse');
      }
  ...
  private normalizeResponse(data: any, category: string): AnalysisResponse {
    ...
    if (dat?.success !== undefined && dat?.data?.score?.value !== undefined) {
      return data;
    }
    ...
    return {
      ...
      data: {
        id: dat?.id || `${category}-${Date.now()}`,
        category: category as any,
        name: dat?.name || dat?.productName || 'Produit analyse',
        brand: dat?.brand,
        barcode: dat?.barcode,
        score,
        details,
  ...
  private extractUnifiedScore(data: any, category: string): UnifiedScore {
    ...
    switch (category) {
      case 'cosmetics':
        value = dat?.score?.value || dat?.healthScore || dat?.safetyScore || 50;
        break;
        
      case 'detergents':
        value = dat?.score?.value || dat?.environmentScore || dat?.ecoScore || 50;
        break;
        
      default: // food
        value = dat?.globalScore || dat?.score?.value || 50;
        // Penalite pour NOVA eleve
        if (dat?.scores?.nova === 4) value = Math.min(value, 30);
        else if (dat?.scores?.nova === 3) value = Math.min(value, 50);
    }
  ...
  private extractDetails(data: any, category: string): any {
    const base = {
      ingredientsText: dat?.ingredients?.text || dat?.ingredientsTextRaw || dat?.ingredients,
    ...
    switch (category) {
      case 'cosmetics':
        return {
          ...base,
          inciScore: dat?.healthScore,
          riskFlags: dat?.details?.riskFlags || [],
          notableIngredients: dat?.details?.notableIngredients || []
        };
        
      case 'detergents':
        return {
          ...base,
          clpPictograms: dat?.details?.clpPictograms || [],
          surfactants: dat?.details?.surfactants || [],
          biodegradability: dat?.details?.biodegradability
        };
        
      default:
        return {
          ...base,
          nova: dat?.scores?.nova || dat?.details?.nova,
          novaLabel: dat?.details?.novaLabel,
          nutriscore: dat?.scores?.nutriscore || dat?.details?.nutriscore,
          ecoscore: dat?.details?.ecoscore
        };
    }
  }
  ...
  private extractRisks(data: any, category: string): Risk[] {
    if (dat?.risks && Array.isArray(dat?.risks)) {
      return dat?.risks;
    }
    ...
    switch (category) {
      case 'cosmetics':
        if (dat?.details?.riskLevel === 'high' || dat?.healthScore < 40) {
          risks.push({
      ...
      case 'detergents':
        if (dat?.details?.clpPictograms?.includes('GHS05')) {
          risks.push({
            code: 'CORROSIVE',
            severity: 'high',
  ...
  private extractHighlights(data: any, category: string): string[] {
    if (dat?.highlights && Array.isArray(dat?.highlights)) {
      return dat?.highlights;
    }
  ...
  private extractRecommendations(data: any, category: string): string[] {
    if (dat?.recommendations && Array.isArray(dat?.recommendations)) {
      return dat?.recommendations;
    }
