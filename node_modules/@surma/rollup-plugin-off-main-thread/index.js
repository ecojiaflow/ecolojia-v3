    ...
    async transform(code, id) {
      ...
      for (const match of matchAll(code, workerRegexpForTransform)) {
        ...
        if (workerSource === "import.meta.url") {
          ...
          if (directWorkerFile) {
            const fullMatchWithOpts = `${fullMatch}, â€¦)`;
            const fullReplacement = `new Worker(new URL(${directWorkerFile}, import.meta.url), â€¦)`;
...
This will become a hard error in the future.`,
    ...
    renderChunk(code, chunk, outputOptions) {
      // We donâ€™t need to do any loader processing when targeting ESM format.
      if (isEsmOutput()) return;

      if (outputOptions.banner && outputOptions.banner.length > 0) {
        this.error(
          "OMT currently doesnâ€™t work with `banner`. Feel free to submit a PR at https://github.com/surma/rollup-plugin-off-main-thread"
        );
        return;
      }
      const ms = new MagicString(code);
      ...
      // Prepend loader if itâ€™s an entry point or a worker file
