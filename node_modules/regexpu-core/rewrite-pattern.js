...
const handleLoneUnicodePropertyNameOrValue = (value) => {
	...
	// Itâ€™s not a `General_Category` value, so check if itâ€™s a property
	// of strings.
	try {
		return getUnicodePropertyValueSet('Property_of_Strings', value);
	} catch (exception) {}
	// Lastly, check if itâ€™s a binary property of single code points.
...
const getUnicodePropertyEscapeSet = (value, isNegative, isUnicodeSetIgnoreCase) => {
	...
	return {
		...
		strings: set.strings
			// We need to escape strings like *ï¸âƒ£ to make sure that they can be safely used in unions.
		...
		case 'reference':
			if (item.name) {
				...
				if (config.transform.namedGroups) {
					...
					// This named reference comes before the group where itâ€™s defined,
					// so itâ€™s always an empty match.
					return {
						'type': 'group',
						'behavior': 'ignore',
						'body': [],
