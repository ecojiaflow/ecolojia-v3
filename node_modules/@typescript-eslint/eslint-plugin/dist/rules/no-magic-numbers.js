...
exports.default = (0, util_1.createRule)({
    ...
    create(context, [options]) {
        ...
        return {
            Literal(node) {
                // If itâ€™s not a numeric literal weâ€™re not interested
                if (typeof node.value !== 'number' && typeof node.value !== 'bigint') {
                    return;
                }
                // This will be `true` if weâ€™re configured to ignore this case (eg. itâ€™s
                // an enum and `ignoreEnums` is `true`). It will be `false` if weâ€™re not
                // configured to ignore this case. It will remain `undefined` if this is
                // not one of our exception cases, and weâ€™ll fall back to the base rule.
                let isAllowed;
                ...
                // If weâ€™ve hit a case where the ignore option is true we can return now
                if (isAllowed === true) {
                    return;
                }
