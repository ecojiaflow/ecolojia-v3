...
interface RuleMetaData<TMessageIds extends string> {
     ...
     * - `"suggestion"` means the rule is identifying something that could be done in a better way but no errors will occur if the code isnâ€™t changed.
     * - `"layout"` means the rule cares primarily about whitespace, semicolons, commas, and parentheses, all the parts of the program that determine how the code looks rather than how it executes. These rules work on parts of the code that arenâ€™t specified in the AST.
     */
...
interface RuleModule<TMessageIds extends string, TOptions extends readonly unknown[] = [], TRuleListener extends RuleListener = RuleListener> {
    ...
    /**
     * Function which returns an object with methods that ESLint calls to â€œvisitâ€
