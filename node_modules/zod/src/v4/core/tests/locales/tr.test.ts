...
test("locales - tr", () => {
  ...
  expect(invalidType.error!.issues[0].message).toBe("GeÃ§ersiz deÄŸer: beklenen number, alÄ±nan string");

  const invalidType2 = z.string().safeParse(1);
  expect(invalidType2.error!.issues[0].code).toBe("invalid_type");
  expect(invalidType2.error!.issues[0].message).toBe("GeÃ§ersiz deÄŸer: beklenen string, alÄ±nan number");

  const invalidValue = z.enum(["a", "b"]).safeParse(1);
  expect(invalidValue.error!.issues[0].code).toBe("invalid_value");
  expect(invalidValue.error!.issues[0].message).toBe('GeÃ§ersiz seÃ§enek: aÅŸaÄŸÄ±dakilerden biri olmalÄ±: "a"|"b"');

  const tooBig = z.number().max(10).safeParse(15);
  expect(tooBig.error!.issues[0].code).toBe("too_big");
  expect(tooBig.error!.issues[0].message).toBe("Ã‡ok bÃ¼yÃ¼k: beklenen number <=10");

  const tooSmall = z.number().min(10).safeParse(5);
  expect(tooSmall.error!.issues[0].code).toBe("too_small");
  expect(tooSmall.error!.issues[0].message).toBe("Ã‡ok kÃ¼Ã§Ã¼k: beklenen number >=10");
  ...
  expect(invalidFormatRegex.error!.issues[0].message).toBe("GeÃ§ersiz metin: /abcd/ desenine uymalÄ±");
  ...
  expect(invalidFormatStartsWith.error!.issues[0].message).toBe('GeÃ§ersiz metin: "abcd" ile baÅŸlamalÄ±');

  const notMultipleOf = z.number().multipleOf(3).safeParse(10);
  expect(notMultipleOf.error!.issues[0].code).toBe("not_multiple_of");
  expect(notMultipleOf.error!.issues[0].message).toBe("GeÃ§ersiz sayÄ±: 3 ile tam bÃ¶lÃ¼nebilmeli");
  ...
  expect(unrecognizedKeys.error!.issues[0].message).toBe('TanÄ±nmayan anahtar: "c"');

  const invalidUnion = z.union([z.string(), z.number()]).safeParse(true);
  expect(invalidUnion.error!.issues[0].code).toBe("invalid_union");
  expect(invalidUnion.error!.issues[0].message).toBe("GeÃ§ersiz deÄŸer");
});
