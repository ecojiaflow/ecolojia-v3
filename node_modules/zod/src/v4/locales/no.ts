  ...
  return (issue) => {
    switch (issue.code) {
      ...
      case "invalid_format": {
        ...
        if (_issue.format === "starts_with") return `Ugyldig streng: mÃ¥ starte med "${_issue.prefix}"`;
        if (_issue.format === "ends_with") return `Ugyldig streng: mÃ¥ ende med "${_issue.suffix}"`;
        if (_issue.format === "includes") return `Ugyldig streng: mÃ¥ inneholde "${_issue.includes}"`;
        if (_issue.format === "regex") return `Ugyldig streng: mÃ¥ matche mÃ¸nsteret ${_issue.pattern}`;
        return `Ugyldig ${Nouns[_issue.format] ?? issue.format}`;
      }
      case "not_multiple_of":
        return `Ugyldig tall: mÃ¥ vÃ¦re et multiplum av ${issue.divisor}`;
      case "unrecognized_keys":
        return `${issue.keys.length > 1 ? "Ukjente nÃ¸kler" : "Ukjent nÃ¸kkel"}: ${util.joinValues(issue.keys, ", ")}`;
      case "invalid_key":
