    ...
    switch (issue.code) {
      ...
      case "invalid_value":
        if (issue.values.length === 1)
          return `Virheellinen syÃ¶te: tÃ¤ytyy olla ${util.stringifyPrimitive(issue.values[0])}`;
        return `Virheellinen valinta: tÃ¤ytyy olla yksi seuraavista: ${util.joinValues(issue.values, "|")}`;
      case "too_big": {
      ...
      case "invalid_format": {
        ...
        if (_issue.format === "starts_with") return `Virheellinen syÃ¶te: tÃ¤ytyy alkaa "${_issue.prefix}"`;
        if (_issue.format === "ends_with") return `Virheellinen syÃ¶te: tÃ¤ytyy loppua "${_issue.suffix}"`;
        if (_issue.format === "includes") return `Virheellinen syÃ¶te: tÃ¤ytyy sisÃ¤ltÃ¤Ã¤ "${_issue.includes}"`;
