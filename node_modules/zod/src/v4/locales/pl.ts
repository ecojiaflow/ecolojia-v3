    ...
    switch (issue.code) {
      ...
      case "invalid_value":
        if (issue.values.length === 1)
          return `NieprawidÅ‚owe dane wejÅ›ciowe: oczekiwano ${util.stringifyPrimitive(issue.values[0])}`;
        return `NieprawidÅ‚owa opcja: oczekiwano jednej z wartoÅ›ci ${util.joinValues(issue.values, "|")}`;
      case "too_big": {
      ...
      case "invalid_format": {
        ...
        if (_issue.format === "ends_with") return `NieprawidÅ‚owy ciÄ…g znakÃ³w: musi koÅ„czyÄ‡ siÄ™ na "${_issue.suffix}"`;
        if (_issue.format === "includes") return `NieprawidÅ‚owy ciÄ…g znakÃ³w: musi zawieraÄ‡ "${_issue.includes}"`;
        if (_issue.format === "regex") return `NieprawidÅ‚owy ciÄ…g znakÃ³w: musi odpowiadaÄ‡ wzorcowi ${_issue.pattern}`;
